...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggsave}\NormalTok{(}\StringTok{"figure2.pdf"}\NormalTok{, }\DataTypeTok{width =} \FloatTok{5.6}\NormalTok{, }\DataTypeTok{height =} \DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{geom_boxplot}\NormalTok{(}\DataTypeTok{width =} \FloatTok{0.75}\NormalTok{, }\DataTypeTok{size =} \FloatTok{0.33}\NormalTok{, }\DataTypeTok{outlier.shape =} \OtherTok{NA}\NormalTok{, }\DataTypeTok{fill =} \NormalTok{RColorBrewer::}\KeywordTok{brewer.pal}\NormalTok{(}\DecValTok{3}\NormalTok{, }\StringTok{"Set1"}\NormalTok{)[}\DecValTok{2}\NormalTok{]) +}
...
\StringTok{  }\KeywordTok{scale_fill_brewer}\NormalTok{(}\DataTypeTok{palette =} \StringTok{"Set1"}\NormalTok{) +}\StringTok{ }
\StringTok{  }\KeywordTok{coord_flip}\NormalTok{() +}
\StringTok{  }\KeywordTok{theme_bw}\NormalTok{() +}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}
    \DataTypeTok{y=}\StringTok{'execution time (seconds)'}\NormalTok{,}
...
\NormalTok{hg19 <-}\StringTok{ }\KeywordTok{db_ucsc}\NormalTok{(}\StringTok{"hg19"}\NormalTok{)}
\KeywordTok{tbl}\NormalTok{(hg19, }\StringTok{"refGene"}\NormalTok{)}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 0 imported}
\CommentTok{#> as numeric}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 4 imported}
\CommentTok{#> as numeric}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 5 imported}
\CommentTok{#> as numeric}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 6 imported}
\CommentTok{#> as numeric}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 7 imported}
\CommentTok{#> as numeric}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 8 imported}
\CommentTok{#> as numeric}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 0 imported}
\CommentTok{#> as numeric}
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 4 imported}
...
\CommentTok{#> Warning in .local(conn, statement, ...): Unsigned INTEGER in col 8 imported}
\CommentTok{#> as numeric}
\CommentTok{#> # Source:   table<refGene> [?? x 16]}
\CommentTok{#> # Database: mysql 5.6.26-log [genomep@genome-mysql.cse.ucsc.edu:/hg19]}
\CommentTok{#>      bin         name chrom strand   txStart     txEnd  cdsStart    cdsEnd}
\CommentTok{#>    <dbl>        <chr> <chr>  <chr>     <dbl>     <dbl>     <dbl>     <dbl>}
\CommentTok{#>  1   585    NR_024540  chr1      -     14361     29370     29370     29370}
\CommentTok{#>  2  1759    NR_110022  chrX      + 153991016 154005964 154005964 154005964}
\CommentTok{#>  3   604    NR_033380  chrX      +   2527305   2575270   2575270   2575270}
\CommentTok{#>  4    75    NR_033381  chrY      +   2477305   2506370   2506370   2506370}
\CommentTok{#>  5   915    NM_182508 chr13      +  43355685  43365685  43358203  43362926}
\CommentTok{#>  6   923    NM_181845 chr19      +  44331472  44353050  44337656  44352793}
\CommentTok{#>  7   586    NR_039983  chr1      -    134772    140566    140566    140566}
\CommentTok{#>  8  1964    NR_028327  chr5      + 180750506 180755196 180755196 180755196}
\CommentTok{#>  9   126 NM_001271875 chr17      -  55938603  56032684  55943837  55962925}
\CommentTok{#> 10  2278    NR_125989  chr1      - 222001007 222014008 222014008 222014008}
\CommentTok{#> # ... with more rows, and 8 more variables: exonCount <dbl>,}
\CommentTok{#> #   exonStarts <chr>, exonEnds <chr>, score <int>, name2 <chr>,}
\CommentTok{#> #   cdsStartStat <chr>, cdsEndStat <chr>, exonFrames <chr>}
\end{Highlighting}
\end{Shaded}
